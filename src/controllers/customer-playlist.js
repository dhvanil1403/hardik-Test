...
const createPlaylist = async (req, res) => {
  try {
    ...
    const playlistData = {
      ...
      playlistDescription,
      urls,
      customer_id // Include customer_id in the playlist data
    };
    ...
    // Log action
    await logAction(req, 'createPlaylist', `Playlist created: ${playlistName}`, customer_id);
...
const editPlaylist = async (req, res) => {
  ...
  try {
    ...
    await logAction(req,'editPlaylist',`Playlist edited: ${playlistName}`, customer_id);
...
const showPlaylist = async (req, res) => {
  try {
    // Retrieve the customer_id from session or request (assuming session contains it)
    const customer_id = req.session.user.id;

    // Fetch playlists for the specific customer
    const playlistsData = await playlists.viewPlaylistCustomer(customer_id);

    // Render the page with playlists belonging to the specific customer
    res.render('customer-playlist', { playlists: playlistsData });
  } catch (error) {
...
const showAvailableScreen = async (req, res) => {
  const screens = await screenModel.customergetNotdeletedScreen();
  res.render("customer-screenSelection", { screens });
};


//show customer screen for select screen 
const showCustomerScreens = async (req, res) => {
  try {
    const customer_id = req.session.customer_id; // Ensure customer_id is stored in session
    if (!customer_id) {
      return res.status(401).send("Unauthorized access"); // Handle cases where customer_id is not in the session
    }

    // Fetch screens for the specific customer
    const screens = await screenModel.fetchScreensByCustomer(customer_id);
    
    // Render the page with the customer-specific screens
    res.render("customer-screenSelection", { screens });
  } catch (err) {
    console.error("Error displaying screens for customer:", err);
    res.status(500).send("Error displaying available screens");
  }
};
...
const showAvailableScreenForEditPlaylist = async (req, res) => {
  ...
  const screens = await screenModel.fetchScreensByCustomer(customer_id);
  res.render("c-editscrennSelection", { screens, playlistsAll });
};
...
const getPlaylistById = async (req, res) => {
  ...
  const customer_id = req.session.customer_id; // Assuming customer_id is stored in session
  
  try {
    const playlist = await playlists.getPlaylistById(playlistId);
    const mediafiles = await library.EditPlaylistvMedia(customer_id); // Pass customer_id to filter data
    ...
    res.render('customerNeweditPlaylist', { playlist, mediafiles: convertedMediaFiles });
  } catch (error) {
...
const deletePlaylist = async (req, res) => {
  ...
  try {
    ...
    res.redirect('/customer-playlist');
  } catch (err) {
...
module.exports = {  checkPermission, createPlaylist, showPlaylist, showAvailableScreen, getPlaylistById, showCustomerScreens,
