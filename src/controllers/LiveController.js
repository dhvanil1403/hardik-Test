...
//   if (!user.permissions.includes('lic')) {
//     req.flash('error_msg', 'You do not have permission to create live content.');
//     return res.redirect('/Dashboard/Screens'); // Redirect if permission is missing
//   }

//   const { liveName, liveDescription, liveUrls, screenIDs, pairingCodes, clock, weather, clockRuntime } = req.body;
...
//     !screenIDs ||
//     !pairingCodes ||
//     screenIDs.length !== pairingCodes.length
//   ) {
//     return res.status(400).json({ message: "Missing or inconsistent data." });
//   }
...
//     clock: clock === 'on', // Default is false if unchecked
//     weather: weather === 'on', // Default is false if unchecked
//     clockRuntime: parseInt(clockRuntime, 10) || 0, // Default is 0
// };


//   try {
//     // Call the model to create a new live record
//     await liveModel.createLive(liveData);

//     // Update the screens with the live content (if applicable)
//     await liveModel.updateScreenWithLive(screenIDs, pairingCodes, liveData);
...
//   const {
//       liveName, liveDescription, liveUrls, screenIDs, pairingCodes
//   } = req.body;
...
//       screenID: screenIDs,
//       pairingCode: pairingCodes,
//       clock: liveUrls.clock,
//       weather:liveUrls.weather,
//       clockRuntime: parseInt(liveUrls.clockRuntime, 10) || 0,
//   };

//   console.log('Live Data Prepared for DB:', liveData);
...
const createlive = async (req, res) => {
  ...
  const liveData = {
    ...
    weather: liveUrls.weather,
    clockRuntime: parseInt(liveUrls.clockRuntime, 10) || 0,
    youtube_live: liveUrls.youtube_live ,
    exoplayer: liveUrls.exoplayer ,
    youtube_live_link: liveUrls.youtube_live_link || null,
  };
