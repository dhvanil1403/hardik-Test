...
const handleUploadInDB = async (req, res) => {
  ...
  try {
    if (fileType.startsWith("video/")) {
      await library.uploadMediaInDB(fileUrl, fileType, fileName, duration, fileSizeMB, user.customer_id);
    } else {
      await library.uploadMediaInDB(fileUrl, fileType, fileName, null, fileSizeMB, user.customer_id);
    }
    ...
    console.error("Error occurred:", err.message || JSON.stringify(err)); // More detailed logging
    await logAction('UPLOAD', `Error uploading file: ${fileName}`, req.ip);
    res.status(500).send(`Error uploading file: ${err.message || 'An unknown error occurred'}`);
  }
};
