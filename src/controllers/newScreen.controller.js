...
const getAllScreens = async (req, res) => {
    try {
        // Fetch screens where customer_id is NULL
        const allScreens = await screen.getAllScreens();
    
        // Fetch additional counts and other data
        const onlineScreen = await screen.getNotDeletedScreenCount();
        const screenCount = await screen.getTotalScreenCount();
        const offlineScreen = await screen.getDeletedScreenCount();
        const deletedScreens = await screen.getDeletedScreen();
        ...
        // const screenSlotData = await screen.getScreenSlotData(); // Fetch screen slot data     
        const screenSlotData = (await screen.getScreenSlotData()) || [];

        // console.log("Fetched screen slot data:", screenSlotData);     
        const screenDeviceConfig = await screen.screenDeviceConfig();
    
        // Render the data
        res.render("Screen", {
          message: null,
          allScreens,           // Pass allScreens as 'screens'
          screens: screenSlotData,  // Pass screenSlotData as 'screenSlotData'         
           // Only screens with NULL customer_id
          screenSlotData,
          screenCount,
          onlineScreen,
          offlineScreen,
          ...
          groupscreen,
          screenStatus,
           // Include screenSlotData in the render
          screenDeviceConfig
        });
    } catch (error) {
        console.error("Error fetching all screens:", error);
...
const editScreen = async (req, res) => {
    ...
    try {
        ...
        // Redirect to the screens dashboard with a success message query parameter
        res.redirect("/Dashboard/Screens?message=success&text=Screen edited successfully");
    } catch (error) {
        console.error(error);
        ...
        // Redirect to the screens dashboard with an error message query parameter
        res.redirect("/Dashboard/Screens?message=error&text=Error editing screen");
    }
};
